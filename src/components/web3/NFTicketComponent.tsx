...
function NFTicketComponent(props: any) {
  ...
  let smartContractAddress = "0x93A8fE00B91829763A797E933686318e89401c46";
  ...
  async function getData() {
    ...
    setUserNfts(balance.toString());

    let supply = await smartContractInstance.totalSupply();
    setActualSupply(supply.toString());
  }

  async function getTransfer() {
    smartContractInstance.on(
      "nfticketBought",
      (msg, buyerAddress, nfticketIdBought, event) => {
        let nftEvent = {
          msg: msg,
          buyerAddress: buyerAddress,
          nfticketIdBought: nfticketIdBought,
        };
  ...
  async function mintNftFunc() {
    //provider.waitForTransaction(confirmations)

    const tx = await smartContractInstance
      .connect(signer)
      .mintNFTicket(userAddress, quantityToMint);

    const txReceipt = await tx.wait(1);
    console.log("SE MINÓ.");
    console.log(txReceipt);

    alert("NFTicket minted succesfully.");
  ...
  return (
    <div>
      <div>
        ...
        <div>
          <h3>Actual NFTickets sold: {actualSupply}</h3>
        </div>

        <div>
          <h3>connectedUserNftBalance: {userNfts}</h3>
        </div>
      </div>
    </div>
  );
